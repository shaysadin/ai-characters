...
export class MemoryManager {
    ...
    public async vectorSearch(
        ...
        const similarDocs = await vectorStore.similaritySearch(recentChatHistory, 3, { fileName: characterFileName })
        .catch((err) => {
   ...
   public async writeToHistory(text: string, characterKey: CharacterKey){
    if(!characterKey || typeof characterKey.userId == "undefined"){
        console.log("Character key set incorectly !");
        return "";
    }
   ...
   public async seedChatHistory(
    ...
    if (await this.history.exists(key)){
        console.log("User already has History!");
        return;
    }
